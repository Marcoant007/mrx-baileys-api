...
import { initStore, Store, useSession } from 'mrx-baileys-store';
...
const MAX_RECONNECT_RETRIES = Number(process.env.MAX_RECONNECT_RETRIES || 7);
...
export async function createSession(options: createSessionOptions) {
  ...
  const handleConnectionClose = async () => {
    ...
    setTimeout(() => createSession(options), restartRequired ? 5000 : RECONNECT_INTERVAL);
  };
...
export function getSessionStatus(session: Session) {
  ...
  let phoneNumber = session.user?.id.split(':');
  let number;
...
export function getSession(sessionId: string) {
  try {
    ...
    logger.error("Erro ao encontrar a sessão: " + error);
  }
}
